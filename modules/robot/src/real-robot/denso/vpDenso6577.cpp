...
// const std::string vpDenso6577::CONST_EMC_MARLIN_F033C_WITHOUT_DISTORTION_FILENAME =
// std::string(VISP_VIPER850_DATA_PATH) +
// std::string("/include/const_eMc_MarlinF033C_without_distortion_Denso6577.cnf");

// const std::string vpDenso6577::CONST_EMC_MARLIN_F033C_WITH_DISTORTION_FILENAME =
// std::string(VISP_VIPER850_DATA_PATH) + std::string("/include/const_eMc_MarlinF033C_with_distortion_Denso6577.cnf");

// const std::string vpDenso6577::CONST_EMC_PTGREY_FLEA2_WITHOUT_DISTORTION_FILENAME =
// std::string(VISP_VIPER850_DATA_PATH) +
// std::string("/include/const_eMc_PTGreyFlea2_without_distortion_Denso6577.cnf");

// const std::string vpDenso6577::CONST_EMC_PTGREY_FLEA2_WITH_DISTORTION_FILENAME =
// std::string(VISP_VIPER850_DATA_PATH) + std::string("/include/const_eMc_PTGreyFlea2_with_distortion_Denso6577.cnf");

// const std::string vpDenso6577::CONST_EMC_SCHUNK_GRIPPER_WITHOUT_DISTORTION_FILENAME =
// std::string(VISP_VIPER850_DATA_PATH) + std::string("/include/"
//                                                    "const_eMc_schunk_gripper_without_distortion_Denso6577."
//                                                    "cnf");

// const std::string vpDenso6577::CONST_EMC_SCHUNK_GRIPPER_WITH_DISTORTION_FILENAME =
// std::string(VISP_VIPER850_DATA_PATH) +
// std::string("/include/const_eMc_schunk_gripper_with_distortion_Denso6577.cnf");

// const std::string vpDenso6577::CONST_EMC_GENERIC_WITHOUT_DISTORTION_FILENAME =
// std::string(VISP_VIPER850_DATA_PATH) + std::string("/include/const_eMc_generic_without_distortion_Denso6577.cnf");

// const std::string vpDenso6577::CONST_EMC_GENERIC_WITH_DISTORTION_FILENAME =
// std::string(VISP_VIPER850_DATA_PATH) + std::string("/include/const_eMc_generic_with_distortion_Denso6577.cnf");

// const std::string vpDenso6577::CONST_CAMERA_FILENAME =
// std::string(VISP_VIPER850_DATA_PATH) + std::string("/include/const_camera_Denso6577.xml");



const char *const vpDenso6577::CONST_MARLIN_F033C_CAMERA_NAME = "Marlin-F033C-12mm";
const char *const vpDenso6577::CONST_PTGREY_FLEA2_CAMERA_NAME = "PTGrey-Flea2-6mm";
const char *const vpDenso6577::CONST_SCHUNK_GRIPPER_CAMERA_NAME = "Schunk-Gripper-PTGrey-Flea2-6mm";
const char *const vpDenso6577::CONST_GENERIC_CAMERA_NAME = "Generic-camera";

const vpDenso6577::vpToolType vpDenso6577::defaultTool = vpDenso6577::TOOL_PTGREY_FLEA2_CAMERA;
...
*/
vpDenso6577::vpDenso6577()
  : tool_current(vpDenso6577::defaultTool), projModel(vpCameraParameters::perspectiveProjWithoutDistortion)
...
/*!
  ...
  Initialize the robot with the default tool vpDenso6577::defaultTool.
 */
void vpDenso6577::init(void)
{
  this->init(vpDenso6577::defaultTool);
  return;
}
...
*/
void vpDenso6577::init(const std::string &camera_extrinsic_parameters)
{
...
/*!
  ...
  \sa init(vpDenso6577::vpToolType, const std::string&),
  init(vpDenso6577::vpToolType, const vpHomogeneousMatrix&)

*/
void vpDenso6577::init(vpDenso6577::vpToolType tool, vpCameraParameters::vpCameraParametersProjType proj_model)
{
  ...
  switch (tool) {
  case vpDenso6577::TOOL_MARLIN_F033C_CAMERA: {
    switch (projModel) {
  ...
  case vpDenso6577::TOOL_PTGREY_FLEA2_CAMERA: {
    switch (projModel) {
    case vpCameraParameters::perspectiveProjWithoutDistortion:
  ...
  case vpDenso6577::TOOL_SCHUNK_GRIPPER_CAMERA: {
    switch (projModel) {
    case vpCameraParameters::perspectiveProjWithoutDistortion:
  ...
  case vpDenso6577::TOOL_GENERIC_CAMERA: {
    switch (projModel) {
    case vpCameraParameters::perspectiveProjWithoutDistortion:
  ...
  case vpDenso6577::TOOL_CUSTOM: {
    throw vpRobotException(vpRobotException::badValue,
                           "No predefined file available for a custom tool"
                           "You should use init(vpDenso6577::vpToolType, const std::string&) or"
                           "init(vpDenso6577::vpToolType, const vpHomogeneousMatrix&) instead");
  }
  default: {
...
eMc_TRANS_XYZ  0.05 0.01 0.06
  ...
  \sa init(vpDenso6577::vpToolType,
vpCameraParameters::vpCameraParametersProjType), init(vpDenso6577::vpToolType,
const vpHomogeneousMatrix&)
*/
void vpDenso6577::init(vpDenso6577::vpToolType tool, const std::string &filename)
{
...
*/
void vpDenso6577::init(vpDenso6577::vpToolType tool, const vpHomogeneousMatrix &eMc_)
{
  this->setToolType(tool);
  this->set_eMc(eMc_);
}
...
*/
void vpDenso6577::parseConfigFile(const std::string &filename)
{
...
/*!
  ...
  vpDenso6577::CONST_CAMERA_FILENAME and containing the camera
  parameters.
  ...
  from const_camera_Denso6577.xml config file.
  ...
  int main()
  {
    ...
    // /udd/fspindle/robot/Denso6577/current/include/const_camera_Denso6577.xml
...
void vpDenso6577::getCameraParameters(vpCameraParameters &cam, const unsigned int &image_width,
                                     const unsigned int &image_height) const
{
  ...
  switch (getToolType()) {
  case vpDenso6577::TOOL_MARLIN_F033C_CAMERA: {
    std::cout << "Get camera parameters for camera \"" << vpDenso6577::CONST_MARLIN_F033C_CAMERA_NAME << "\""
      << std::endl
      << "from the XML file: \"" << vpDenso6577::CONST_CAMERA_FILENAME << "\"" << std::endl;
    if (parser.parse(cam, vpDenso6577::CONST_CAMERA_FILENAME, vpDenso6577::CONST_MARLIN_F033C_CAMERA_NAME, projModel,
  ...
  case vpDenso6577::TOOL_PTGREY_FLEA2_CAMERA: {
    std::cout << "Get camera parameters for camera \"" << vpDenso6577::CONST_PTGREY_FLEA2_CAMERA_NAME << "\""
      << std::endl
      << "from the XML file: \"" << vpDenso6577::CONST_CAMERA_FILENAME << "\"" << std::endl;
    if (parser.parse(cam, vpDenso6577::CONST_CAMERA_FILENAME, vpDenso6577::CONST_PTGREY_FLEA2_CAMERA_NAME, projModel,
                     image_width, image_height) != vpXmlParserCamera::SEQUENCE_OK) {
      throw vpRobotException(vpRobotException::readingParametersError, "Impossible to read the camera parameters.");
    }
    break;
  ...
  case vpDenso6577::TOOL_SCHUNK_GRIPPER_CAMERA: {
    std::cout << "Get camera parameters for camera \"" << vpDenso6577::CONST_SCHUNK_GRIPPER_CAMERA_NAME << "\""
      << std::endl
      << "from the XML file: \"" << vpDenso6577::CONST_CAMERA_FILENAME << "\"" << std::endl;
    if (parser.parse(cam, vpDenso6577::CONST_CAMERA_FILENAME, vpDenso6577::CONST_SCHUNK_GRIPPER_CAMERA_NAME, projModel,
                     image_width, image_height) != vpXmlParserCamera::SEQUENCE_OK) {
      throw vpRobotException(vpRobotException::readingParametersError, "Impossible to read the camera parameters.");
    }
    break;
  ...
  case vpDenso6577::TOOL_GENERIC_CAMERA: {
    std::cout << "Get camera parameters for camera \"" << vpDenso6577::CONST_GENERIC_CAMERA_NAME << "\"" << std::endl
      << "from the XML file: \"" << vpDenso6577::CONST_CAMERA_FILENAME << "\"" << std::endl;
    if (parser.parse(cam, vpDenso6577::CONST_CAMERA_FILENAME, vpDenso6577::CONST_GENERIC_CAMERA_NAME, projModel,
                     image_width, image_height) != vpXmlParserCamera::SEQUENCE_OK) {
      throw vpRobotException(vpRobotException::readingParametersError, "Impossible to read the camera parameters.");
    }
    break;
  ...
  default: {
    ...
    //        "vpDenso6577::vpDenso6577ToolType, et controlez que "
  ...
  case vpDenso6577::TOOL_MARLIN_F033C_CAMERA: {
    ...
    if (image_width == 640 && image_height == 480) {
      std::cout << "Get default camera parameters for camera \"" << vpDenso6577::CONST_MARLIN_F033C_CAMERA_NAME << "\""
        << std::endl;
  ...
  case vpDenso6577::TOOL_SCHUNK_GRIPPER_CAMERA: {
    // Set default intrinsic camera parameters for 640x480 images
    if (image_width == 640 && image_height == 480) {
      std::cout << "Get default camera parameters for camera \"" << vpDenso6577::CONST_PTGREY_FLEA2_CAMERA_NAME << "\""
        << std::endl;
      switch (this->projModel) {
      case vpCameraParameters::perspectiveProjWithoutDistortion:
  ...
  case vpDenso6577::TOOL_GENERIC_CAMERA: {
    // Set default intrinsic camera parameters for 640x480 images
    if (image_width == 640 && image_height == 480) {
      std::cout << "Get default camera parameters for camera \"" << vpDenso6577::CONST_GENERIC_CAMERA_NAME << "\""
        << std::endl;
      switch (this->projModel) {
      case vpCameraParameters::perspectiveProjWithoutDistortion:
        cam.initPersProjWithoutDistortion(868.0, 869.0, 314.8, 254.1);
...
*/
void vpDenso6577::getCameraParameters(vpCameraParameters &cam, const vpImage<unsigned char> &I) const
{
...
void vpDenso6577::getCameraParameters(vpCameraParameters &cam, const vpImage<vpRGBa> &I) const
{
  getCameraParameters(cam, I.getWidth(), I.getHeight());
}
END_VISP_NAMESPACE
