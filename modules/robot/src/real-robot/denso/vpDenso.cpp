...
#include <visp3/robot/vpDenso.h>

BEGIN_VISP_NAMESPACE
const unsigned int vpDenso::njoint = 6;
 ...
 * Copy constructor.
 */
vpDenso::vpDenso(const vpDenso &viper)
{
  *this = viper;
}
 ...
 * Copy operator.
 */
vpDenso &vpDenso::operator=(const vpDenso &viper)
{
  eMc = viper.eMc;
  etc = viper.etc;
  erc = viper.erc;
...
*/
vpHomogeneousMatrix vpDenso::getForwardKinematics(const vpColVector &q) const
{
 ...
 */
bool vpDenso::convertJointPositionInLimits(unsigned int joint, const double &q, double &q_mod,
...
*/
unsigned int vpDenso::getInverseKinematicsWrist(const vpHomogeneousMatrix &fMw, vpColVector &q,
                                                const bool &verbose) const
{
  vpColVector q_sol[8];
...
*/
unsigned int vpDenso::getInverseKinematics(const vpHomogeneousMatrix &fMc, vpColVector &q, const bool &verbose) const
{
  vpHomogeneousMatrix fMw;
...
*/
vpHomogeneousMatrix vpDenso::get_fMc(const vpColVector &q) const
{
  vpHomogeneousMatrix fMc;
  get_fMc(q, fMc);
...
/*!
  ...
  \sa get_fMe(), get_eMc()
*/
void vpDenso::get_fMc(const vpColVector &q, vpHomogeneousMatrix &fMc) const
{
...
*/
void vpDenso::get_wMe(vpHomogeneousMatrix &wMe) const
{
...
/*!
  ...
  \sa get_cMe()
*/
void vpDenso::get_eMc(vpHomogeneousMatrix &eMc_) const { eMc_ = this->eMc; }
...
*/
void vpDenso::get_eMs(vpHomogeneousMatrix &eMs) const
{
  eMs.eye();
  eMs[2][3] = -d7; // tz = -d7
}
...
/*!
  ...
  \sa get_eMc()
*/
void vpDenso::get_cMe(vpHomogeneousMatrix &cMe) const { cMe = this->eMc.inverse(); }
...
*/
void vpDenso::get_cVe(vpVelocityTwistMatrix &cVe) const
{
...
/*!
  ...
  \sa get_fJw()
*/
void vpDenso::get_eJe(const vpColVector &q, vpMatrix &eJe) const
{
  vpMatrix V(6, 6);
  V = 0;
...
void vpDenso::get_fJw(const vpColVector &q, vpMatrix &fJw) const
{
  double q1 = q[0];
  double q2 = q[1];
  double q3 = q[2];
...
/*!
  ...
  \sa get_fJw
*/
void vpDenso::get_fJe(const vpColVector &q, vpMatrix &fJe) const
{
  vpMatrix V(6, 6);
  V = 0;
...
*/
vpColVector vpDenso::getJointMin() const { return joint_min; }
...
*/
vpColVector vpDenso::getJointMax() const { return joint_max; }
...
*/
double vpDenso::getCoupl56() const { return c56; }
...
/*!
  ...
  and the tool frame.
*/
void vpDenso::set_eMc(const vpHomogeneousMatrix &eMc_)
{
  this->eMc = eMc_;
...
*/
void vpDenso::set_eMc(const vpTranslationVector &etc_, const vpRxyzVector &erc_)
{
  this->etc = etc_;
  this->erc = erc_;
...
*/
VISP_EXPORT std::ostream &operator<<(std::ostream &os, const vpDenso &viper)
{
