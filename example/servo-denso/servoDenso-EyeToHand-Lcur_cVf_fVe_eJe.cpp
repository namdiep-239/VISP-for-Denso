...
/*!
  \example servoDensoIBVS-EyeToHand-Lcur_cVf_fVe_eJe.cpp

  Example of eye-to-hand image-based control law. We control here a real robot, the
  Denso Emika Panda robot (arm with 7 degrees of freedom).
...
#endif
// Read and save desired features to a file
...
// Read desired features from a file
...
int main(int argc, char **argv)
{
  ...
  if (opt_intrinsic_filename.empty()) {
    std::cout << "Don't have camera intrinsic parameters." << std::endl;
    return EXIT_FAILURE;
  }
  ...
  //#######################################################################
  // Setup Apriltag detector and feature points
  ...
  // TODO: We haven't had r_P_c yet
  // r_P_c transformation between the robot reference and the camera frames
  // e_P_o transformation between the robot end-effector and the object (tag) frames
  ...
  try {
    ...
    robot.setRobotState(vpRobot::STATE_POSITION_CONTROL); // Set the robot in position control mode to move it to the initial position before starting the visual servoing loop
    ...
    std::cout << "Denso exception: " << e.what() << std::endl;
    return EXIT_FAILURE;
  }
  return EXIT_SUCCESS;
}
