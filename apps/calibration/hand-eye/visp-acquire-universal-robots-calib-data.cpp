...
//---------------------------------------------------
    ...
    std::shared_ptr<vpDisplay> disp_left;
    std::shared_ptr<vpDisplay> disp_right;

#else

    vpDisplay *disp_left;
    vpDisplay *disp_right;
    ...
    while (true) {
      ...
      cv::Mat left_cv =
        gray(cv::Rect(0, 0, half_width, height));

      cv::Mat right_cv =
        gray(cv::Rect(half_width, 0, half_width, height));

      //-------------------------------------------------
      // OpenCV -> ViSP
      //-------------------------------------------------

      vpImageConvert::convert(left_cv, I_left);
      vpImageConvert::convert(right_cv, I_right);
      ...
      if (vpDisplay::getClick(I_left, button, false)) {
        ...
        if (button == vpMouseButton::button1) {
          ...
          vpImageIo::write(I_left, ss_left.str());
          vpImageIo::write(I_right, ss_right.str());
